...
* [两个链表中的第一个公共节点](#面试题52_两个链表中的第一个公共节点)
* [在排序数组中查找数字](#面试题53_在排序数组中查找数字)
* [二叉树的深度](#面试题55_二叉树的深度)
* [数组中只出现一次的数字](#数组中只出现一次的两个数字)
...
* [面试题52_两个链表中的第一个公共节点](#面试题52_两个链表中的第一个公共节点)
* [面试题53_在排序数组中查找数字](#面试题53_在排序数组中查找数字)
* [面试题55_二叉树的深度](#面试题55_二叉树的深度)
...
1.很容易想到先获取两个链表的长度，再让长的链表指针先走"几步"(其中"几"为两个链表的长度差)，再使得两个链表的指针同时走，当获取到同一个节点的时候返回
...
public class Solution {
    public ListNode FindFirstCommonNode(ListNode pHead1, ListNode pHead2) {
        ...
        int num1 = 0;
        int num2 = 0;
        ListNode first1 = pHead1; 
        while(first1!=null){
            num1++;
            first1 = first1.next;
        }
        ListNode first2 = pHead2; 
        while(first2!=null){
            num2++;
            first2 = first2.next;
        }
...
2.一个使代码更加简洁的想法为两个指针同时移动，当指针遍历到链表结尾null的时候就开始遍历另一个链表，这样的话两个指针遍历的长度一致，会同时遍历到共同节点
...
public class Solution {
    public ListNode FindFirstCommonNode(ListNode pHead1, ListNode pHead2) {
        ...
        while(p1!=p2){
            ...
            if(p1==pHead2 && ++num==3)    return null;  
        }
        return p1;
    }
}
...
其中num的作用是为了防止两个链表无重复节点而使程序陷入死循环，按理说num设置为2就可以了但是通不过，这是为什么捏？
...
1.利用二分查找，找出数字，再在数字两边开始搜索

2.利用二分查找，找出第一次出现的位置以及最后出现的位置，两者做差加一

3.找出该数字+0.5与该数字-0.5的位置，两者相减再减一
...
public class Solution {
    public int GetNumberOfK(int [] array , int k) {
        int res = 0;
       int index = GetIndexOfK(array, k, 0, array.length-1);
        if(index!=-1)
            res++;
        else
            return 0;
        int pre = index;
        int pos = index;
       while(pre >=1 && array[--pre]==k){
           res++;
           if(array[pre]!=k) 
               break;
       }
        while(pos <=(array.length-2) && array[++pos]==k){
            res++;
            if(array[pos]!=k)
                break;
        }
    ...
    private int GetIndexOfK(int[] array, int k, int lo, int hi){
        if(lo>hi)
            return -1;
        int mid = lo+(hi-lo)/2;
        if(array[mid]==k)
            ...
            return GetIndexOfK(array, k, lo, mid-1);
        else
            return GetIndexOfK(array, k, mid+1, hi);
...
public class Solution {
    ...
    private int getFirstIndex(int [] arr, int k, int lo, int hi){
        if(lo>hi)
            return -1;
        int mid = lo+((hi-lo)>>1);
        if(arr[mid]>k)
            return getFirstIndex(arr, k, lo, mid-1);
        else if(arr[mid]<k)
            return getFirstIndex(arr, k, mid+1, hi);
        else if((mid-1)>=0 && arr[mid-1]==k)
            return getFirstIndex(arr, k, lo, mid-1);
        else
            return mid;
    }
    private int getLastIndex(int [] arr, int k, int lo, int hi){
        if(lo>hi)
            return -1;
        int mid = lo+(hi-lo)/2;
        if(arr[mid]>k)
            return getLastIndex(arr, k, lo, mid-1);
        else if(arr[mid]<k)
            return getLastIndex(arr, k, mid+1, hi);
...
public class Solution {
    public void FindNumsAppearOnce(int [] array,int num1[] , int num2[]) {
        ...
        int mul = 2,res0=0,res1=0;
        while(res%mul==0){
            mul*=2;
        }
        for(int i=0;i<array.length;i++){
            int num = array[i];
            if(num%mul==0)        res0^=num;
            else                  res1^=num;
        }
        num1[0] = res0;
        num2[0] = res1;
